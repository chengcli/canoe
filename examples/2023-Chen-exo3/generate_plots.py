...
variables_to_plot = ['temp_avg', 'vlat_avg', 'vlon_avg', 'upvp', 'upwp', 'uv_variance']  # Specify the variable names
...
with Dataset(nc_file_path, mode='r') as nc:
    ...
    for var_name in variables_to_plot:
        ...
        contour = plt.contourf(X, Y, data, levels=np.linspace(colorbar_ranges[var_name][0], colorbar_ranges[var_name][1], 100), cmap=colormaps[var_name], extend='both')
        ...
        cbar.ax.set_yticklabels([f'{tick:.0f}' for tick in colorbar_ticks[var_name]])  # Set tick labels with desired format
