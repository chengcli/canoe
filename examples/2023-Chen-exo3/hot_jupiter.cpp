...
// \brief Held-Suarez atmosphere benchmark test. Refernce: Held & Suarez, (1994).
// Forcing parameters are given in the paper.
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
    for (int j = pmb->js; j <= pmb->je; ++j)
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        du(IEN,k,j,i) += -dt*(cp-Rd)*w(IDN,k,j,i)*(temp - T_eq)/Kt;
      }
...
void MeshBlock::ProblemGenerator(ParameterInput *pin)
{
  ...
  //            (Rd* ( temp + 20.*(distribution(generator) - 0.5) * (1. + cos(k3*phi)) ) );
