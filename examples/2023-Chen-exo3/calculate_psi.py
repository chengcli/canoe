...
with nc.Dataset(infile, 'r') as src:
    ...
    for i in range(1, len(x1)):
        for j in range(len(lat)):
            psi[:, i, j, :] = psi[:, i-1, j, :] + vlon[:, i, j, :] * (x1[i] - x1[i-1]) * rho[:, i, j, :] * dlon * np.pi / 180.0 * Rp * np.cos(lat[j] * np.pi / 180.0)
    for i in range(1, len(lat)):
        psi[:, :, i, :] = psi[:, :, i-1, :] - vel1[:, :, i, :] * (lat[i] - lat[i-1]) * np.pi / 180.0 * Rp * rho[:, :, i, :] * dlon * np.pi / 180.0 * Rp * np.cos(lat[i] * np.pi / 180.0)
