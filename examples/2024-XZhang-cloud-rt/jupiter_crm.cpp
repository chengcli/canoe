...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin) {
  ...
  SetUserOutputVariableName(4, "lat");
  SetUserOutputVariableName(5, "lon");
  SetUserOutputVariableName(6, "U");
  SetUserOutputVariableName(7, "V");
  ...
  for (int k = ks; k <= ke; ++k)
    for (int j = js; j <= je; ++j)
      for (int i = is; i <= ie; ++i) {
        ...
        user_out_var(4, k, j, i) = lat / PI * 180.0;
        user_out_var(5, k, j, i) = lon / PI * 180.0;
        Real U, V;
        pimpl->pexo3->GetUV(&U, &V, Vy, Vz, k, j, i);
        user_out_var(6, k, j, i) = U;
        user_out_var(7, k, j, i) = V;
      }
}
