...
Real Omega_f, center_deg, beta_f;
int iH2O, iNH3, iNH4SH;
  ...
  for (int k = pmb->ks; k <= pmb->ke; ++k)
    for (int j = pmb->js; j <= pmb->je; ++j)
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        // Load Latitude (constant f here)
        Real theta0 = center_deg * M_PI / 180.;
        // Coriolis force (shallow approximation)
        Real local_f = 2. * sin(theta0) * Omega_f + x2 * beta_f;
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
  ...
  Omega_f = pin->GetReal("problem", "Omega_f");

  // index
  auto pindex = IndexMap::GetInstance();
  iH2O = pindex->GetVaporId("H2O");
  iNH3 = pindex->GetVaporId("NH3");
  iNH4SH = pindex->GetVaporId("NH4SH");
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  Real xNH4SH = pin->GetReal("problem", "qNH4SH.ppmv") / 1.E6;
