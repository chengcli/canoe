                          ...
                          int k, int j, int i)
{
  if (NCHEMISTRY > 0) {
     for (int n=0; n<NCHEMISTRY; ++n) {
        vmr[n] = prim_scalar(NCLOUD + n, k, j, i)/vmass[n]*mmass; //change mmr
to vmr
      }
    }
}
                        ...
                        int k, int j, int i)
{
  if (NCHEMISTRY > 0) {
     for (int n=0; n<NCHEMISTRY; ++n) {
        cons_scalar(NCLOUD + n, k, j, i) = w(IDN, k, j,
i)*vmr[n]*vmass[n]/mmass; //change vmr to den
      }
    }
}
*/
    ...
    for (int j = pmb->js; j <= pmb->je; ++j)
      for (int i = pmb->is; i <= pmb->ie; ++i) {
        ...
        /*
                ...
                if (i > pmb->is+10) {
                  for (int n = 0; n < NCHEMISTRY; ++n) {
                     cons_scalar(NCLOUD + n, k, j, i) -= rho *
           prim_scalar(NCLOUD + n, k, j, i) * n * dt / 1.E20;
                  }
                } else {
                  for (int n = 0; n < NCHEMISTRY; ++n) {
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
  ...
  for (int k = ks; k <= ke; k++)
    for (int j = js; j <= je; j++)
      for (int i = is; i <= ie; i++) {
        double mu;
        double T_in = pthermo->GetTemp(w.at(k, j, i));
