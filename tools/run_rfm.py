...
def RunRfm(
    hitfile, wmin, wmax, wdel, tmin, tmax, tnum, molecules, atm_data, rundir="."
):
    pwd = os.getcwd()
    # create rfm run file
    driver = OrderedDict(
...
def CreateKcoeffInp(
    file, wmin, wmax, wdel, tmin, tmax, tnum, molecules, atm_data, rundir="."
):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hitbin", default="hitbin", help="hitbin exe")
    parser.add_argument("--rfm", default="rfm", help="rfm exe")
    parser.add_argument(
        "--par", default="", help="hitran par file, must specify if no hit"
    )
    parser.add_argument(
        "--hit", default="", help="hitran hit file, must specify if no par"
    )
    parser.add_argument("--atm", default="atm.txt", help="atmosphere profile")
    parser.add_argument("--wave", default="0. 0. 1", help="wavenumber grid")
    parser.add_argument(
        "--temp",
        default="-5. 5. 3",
        help="temperature grid",
    )
    parser.add_argument("--molecule", default="", help="molecular absorber")
    parser.add_argument("--output", default="stdout", help="kcoeff output file")
    parser.add_argument("--version", default="12", help="Hitran version")
    parser.add_argument("--rundir", default=".", help="direcotry contains tab files")
