...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < nmols; ++i) {
    nc_def_dim(ncid, ("T_" + mols[i]).c_str(), temp_axis.size(), temp_dimid + i);
    nc_def_var(ncid, ("T_" + mols[i]).c_str(), NC_DOUBLE, 1, temp_dimid + i, temp_varid + i);
  }
