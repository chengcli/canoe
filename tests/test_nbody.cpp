...
void nbody_direct(vecd& sl_coord, vecd& sl_den,
           vecd& dl_coord, vecd& dl_den,
           ...
           const pvfmm::Kernel<double>& kernel_fn)
{
  ...
  MPI_Allreduce(&n_trg, &n_trg_glb, 1, MPI_LONG_LONG, MPI_SUM,
      MPI_COMM_WORLD);
  ...
  MPI_Allgather(&send_cnt, 1, MPI_INT, recv_cnts.data(), 1, MPI_INT,
      MPI_COMM_WORLD);
  MPI_Allgatherv(trg_coord.data(), send_cnt, MPI_DOUBLE,
                 glb_trg_coord.data(), recv_cnts.data(), recv_disp.data(),
  ...
  for (int i = 0; i < nthreads; ++i) {
    ...
    if (kernel_fn.ker_poten != NULL) {
      kernel_fn.ker_poten(sl_coord.data(), n_sl,
                          sl_den.data(), 1,
    ...
    if (kernel_fn.dbl_layer_poten != NULL) {
      kernel_fn.dbl_layer_poten(dl_coord.data(), n_dl,
                                dl_den.data(), 1,
                                glb_trg_coord.data() + a * PVFMM_COORD_DIM, b - a,
                                glb_trg_value_.data() + a * kernel_fn.ker_dim[1],
                                NULL);
    ...
    MPI_Allreduce(glb_trg_value_.data(), 
                  glb_trg_value.data(), glb_trg_value.size(),
                  MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

    // set local target values
    int disp = recv_disp[Globals::my_rank] / PVFMM_COORD_DIM;
    trg_value.assign(glb_trg_value.begin() + disp * kernel_fn.ker_dim[1],
...
TEST(fmm_laplace, potential) {
  ...
  { // Direct N-Body
    ...
    nbody_direct(sl_coord, sl_den , dl_coord, dl_den,
                 trg_coord, trg_value_direct, kernel_fn);
