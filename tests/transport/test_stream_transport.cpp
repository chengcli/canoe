...
void set_boundary_condition(AthenaArray<Real> &data) {
  ...
  for (int j = 0; j < rowsh; ++j) {
    for (int i = 1; i <= GhostZoneSize; ++i) data(j, is - i) = data(j, is);
    for (int i = 1; i <= GhostZoneSize; ++i) data(j, ie + i) = data(j, ie);
  }
  for (int i = 0; i < colsh; ++i) {
    for (int j = 1; j <= GhostZoneSize; ++j) data(js - j, i) = data(js, i);
    for (int j = 1; j <= GhostZoneSize; ++j) data(je + j, i) = data(je, i);
  }
  /*for (int i = 0; i < colsh; ++i) {
    data(0,i) = data(1,i);
...
int main(int argc, char **argv) {
  ...
  for (int j = 0; j < rowsh; ++j)
    for (int i = 0; i < colsh; ++i) {
      streamf(j, i) = -(j + 1) * dx;
      // streamf(j,i) = 0.;
      if (j > rowsh / 4. && j < 3. * rowsh / 4. && i > colsh / 4. &&
          i < 3. * colsh / 4.)
        data(j, i) = 1.;
    }
  ...
  printf("data = \n");
  for (int j = 0; j < rowsh; ++j) {
    for (int i = 0; i < colsh; ++i) printf("%7.2f", data(j, i));
    printf("\n");
  }
  for (int j = js; j <= je; ++j)
    for (int i = is; i <= ie; ++i) sum += data(j, i);
  printf("sum = %7.2f\n", sum);
  printf("streamf = \n");
  for (int j = 0; j < rowsh; ++j) {
    for (int i = 0; i < colsh; ++i) printf("%7.2f", streamf(j, i));
    printf("\n");
  }
  printf("\n");
