...
template <typename T>
std::array<T, 3> vec_raise(StrideIterator<T*> w, StrideIterator<T*> m) {
  std::array<T, 3> v{w[IVX], w[IVY], w[IVZ]};
  return v;
}
...
template <typename T>
std::array<T, 3> vec_lower(StrideIterator<T*> w, StrideIterator<T*> m) {
  std::array<T, 3> v{w[IVX], w[IVY], w[IVZ]};
  return v;
}
