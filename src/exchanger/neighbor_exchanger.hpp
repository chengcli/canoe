...
void NeighborExchanger<T>::Transfer(MeshBlock const *pmb, int n) {
  for (auto &nb : pmb->pbval->neighbor) {
    if (nb.snb.rank == Globals::my_rank) {  // on the same process
      ...
      int tag = MessageHelper::create_mpi_tag(nb.snb.lid, nb.targetid,
                                              MessageTraits<T>::name);
  ...
  for (auto &nb : pmb->pbval->neighbor) {
    ...
    int tag = MessageHelper::create_mpi_tag(pmb->lid, nb.bufid,
                                            MessageTraits<T>::name);

    MPI_Probe(nb.snb.rank, tag, MPI_COMM_WORLD, &status);
    MPI_Get_count(&status, MessageTraits<T>::mpi_type, &rsize);

    recv_buffer_[nb.bufid].resize(rsize);
    MPI_Irecv(recv_buffer_[nb.bufid].data(), rsize, MessageTraits<T>::mpi_type,
