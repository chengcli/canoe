...
//========================================================================================
//! \file hllc.cpp
//  \brief HLLC Riemann solver for hydrodynamics, an extension of the HLLE
//  fluxes to include the contact wave.  Only works for adiabatic hydrodynamics.
//
// REFERENCES:
...
//
// - P. Batten, N. Clarke, C. Lambert, and D. M. Causon, "On the Choice of
// Wavespeeds
//   for the HLLC Riemann Solver", SIAM J. Sci. & Stat. Comp. 18, 6, 1553-1570,
//   (1997).
...
#pragma omp simd private(wli, wri, flxi, fl, fr)
  for (int i = il; i <= iu; ++i) {
    ...
    for (int n = 1; n <= NVAPOR; ++n) flxi[n] = sl * fl[n] + sr * fr[n];
    ...
    for (int n = 1; n <= NVAPOR; ++n) flx(n, k, j, i) = flxi[n];
