...
double absorption_coefficient_PH3_radtran(double freq, double P, double T, double XH2,
       double XHe, double XPH3)
{
   double dtaua = 0.0;
   ...
   for (int i=0; i<NLINE_PH3; i++) {
      ...
      double tx35 = tx*tx*tx*sqrt(tx);  // was pow(tx,texp), but gnu pow() is very slow
      double boltzf = exp(hc_over_kTref*elow*(1.0-tx));
      dtaua += acon*PPH3*intens*tx35*boltzf*fx*fline;
   }

   if (iph3cor)
      dtaua *= 35.386 * pow(freq,-0.7969);

   return dtaua; // in cm^-1
}


double absorption_coefficient_PH3_Hoffman(double freq, double P, double T, double XH2,
       double XHe, double XPH3)
{
   double const factor = 241.43; // bar/P_cgs * 1/kb_cgs * (cm/nm)^2 / (300K) * (GHz/MHz)
   ...
   for (int i=0; i<NLINE_PH3; i++) {
      ...
      if (i<40 && (std::abs(ph3_lines[i].QNp[1])==6 ||
                   (std::abs(ph3_lines[i].QNp[1])==3 && std::abs(ph3_lines[i].QNp[0])<8))) {
         intens *= 2.78;
         gamma_H2 = 1.4121;
         gamma_He = 0.7205;
         gamma_PH3 = 0.4976;
       ...
       else if (i<40 && (std::abs(ph3_lines[i].QNp[1])==3 &&
                         std::abs(ph3_lines[i].QNp[0])>=8) && std::abs(ph3_lines[i].QNp[0])<=26) {
          intens *= 36.65;
          gamma_H2 = 0.5978;
          gamma_He = 0.3050;
          gamma_PH3 = 3.1723;
      ...
      double fx = freq / frq0;
      double delnu = PPH3*gamma_PH3*tx + pow(tx,0.75)*(PH2*gamma_H2 + PHe*gamma_He);

      double delnu_sq = SQUARE(delnu);
      double fline = fx/pi*delnu*(1.0/(SQUARE(freq-frq0) + delnu_sq) +
