...
inline double Lineshape_BR(double obsfreq, double linefreq, double gamma, double zeta, double delta)
{
   ...
   u2 = SQUARE(obsfreqghz) - SQUARE(linefreqghz+delta) - SQUARE(gamma) + SQUARE(zeta);
 ...
 */
   ...
   for (auto I=0; I<Nlines; I++) {
      ...
      A *= exp( -(h*c*h2s_lines[I].ELO/kb)*(1.0/T_in - 1.0/To_intensity) );
