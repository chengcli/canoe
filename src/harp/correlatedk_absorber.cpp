...
//  std::cout << " len: " << len_[0] <<" "<< len_[1] <<" "<< len_[2] <<" "<<
//  len_[3] <<std::endl; std::cout << " axislen: " <<axis_[len_[0]]<<std::endl;
//  for (int i = 1; i < 20; ++i) {
...
Real CorrelatedKAbsorber::getAttenuation(Real g1, Real g2,
  ...
  /*
  //  std::cout << " len: " << len_[0] <<" "<< len_[1] <<" "<< len_[2] <<" "<<
  len_[3] <<std::endl; std::cout << " axis: " << axis_[len_[0] + len_[1] + mw]
  <<" "<< axis_[len_[0] + len_[1] + len_[2] + mg] <<" "<< mw <<" "<< mg
  <<std::endl; std::cout << " inter: " << coord[0] <<" "<< coord[1] <<" "<<
  coord[2] <<" "<< coord[3] <<std::endl; for (int i = 0; i < 10; ++i) { Real yy
  = float(i+1)*200.; coord[0] = log(1.E5); coord[1] = yy; interpn(&val, coord,
  kcoeff_.data(), axis_.data(), len_, 4); std::cout << " test: " << yy <<"
  "<<exp(val) <<" "<<std::endl;
    }
  */
