  ...
  for (auto a : absorbers) {
    for (int i = il; i <= iu; ++i) {
      ...
      for (int m = 0; m < nspec; ++m) {
        Real kcoeff = a->getAttenuation(spec_[m].wav1, spec_[m].wav2, var);  // 1/m
        Real dssalb = a->getSingleScatteringAlbedo(spec_[m].wav1, spec_[m].wav2, var)*kcoeff;
        // tau 
        tau_(m,i) += kcoeff;
        // ssalb
        ssa_(m,i) += dssalb;
        // pmom
        a->getPhaseMomentum(mypmom.data(), spec_[m].wav1, spec_[m].wav2, var, npmom);
  ...
  for (int m = 0; m < nspec; ++m) {
    for (int i = il; i <= iu; ++i) {
      if (tau_(m,i) > 1e-6 && ssa_(m,i) > 1e-6) {  // has scattering
        ...
        tau_(m,i) *= pcoord_->dx1f(i)*w(IPR,k,j,i)/(w(IDN,k,j,i)*grav*Constants::PressureScaleHeight);
#else
