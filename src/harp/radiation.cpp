...
void Radiation::calculateRadiativeFlux(AthenaArray<Real> &flxup, AthenaArray<Real> &flxdn,
  Real time, int k, int j, int il, int iu)
{
  ...
  for (auto p : bands) {
    ...
    if (p->test(RadiationFlags::Dynamic)) {
      planet_->ParentZenithAngle(&ray.mu, &ray.phi, time, pcoord_->x2v(j), pcoord_->x3v(k));
...
void Radiation::calculateRadiance(AthenaArray<Real> &radiance, Real time,
...
void Radiation::addRadiativeFlux(Hydro *phydro,
  int k, int j, int il, int iu) const
{
...
#pragma omp simd
    for (int i = il; i <= iu; ++i)
      phydro->flux[X1DIR](IEN,k,j,i) += flxup(b,k,j,i) - flxdn(b,k,j,i);
  }

  pdebug->Leave();
}

void Radiation::readRadiationBands(MeshBlock *pmb, ParameterInput *pin, int &bid)
{
  char name[80];
  while (true) {
  ...
  if (pin->DoesParameterExist("radiation", "bandsfile")) {
    ...
    IOWrapper infile;
    infile.Open(pin->GetString("radiation", "bandsfile").c_str(), IOWrapper::FileMode::read);
