...
void usage();
int process_file(char *, unsigned char, struct fileinfo *, char *, int *, int *,
                 int, int, void *[], int, unsigned char, unsigned char);
...
static unsigned char _first_; /* First time reading variables? */
int mppnccombine(int argc, char *argv[]) {
  ...
  /* Open an existing netCDF file for appending */
    ...
    if ((ncoutfile->ncfid = ncopen(outfilename, NC_WRITE)) == (-1)) {
      fprintf(stderr,
              "Error: cannot open the output netCDF file for appending!\n");
      free(ncoutfile);
      return (1);
    }
  }

  for (f = 0; f < NC_MAX_VARS; f++) varbuf[f] = NULL;
        ...
        for (a = nstart; a <= nend; a++) {
          ...
          if (verbose) {
            if (a == nstart && r == 0)
              ...
              printf("  %d files to go... ", nend - nstart + 1 - f);
            printf("processing \"%s\"\n", infilename);
          }
  ...
  /* Loop over all the specified input files */
    ...
    for (r = 0; r < nrecs; r++) {
      ...
      for (a = inputarg; a < argc; a++) {
        ...
        infileerror =
            process_file(argv[a], appendnc, ncoutfile, outfilename, &nfiles,
                         &nrecs, r, f, varbuf, headerpad, verbose, missing);
        if (infileerror) infileerrors = 1;
        appendnc = 1;
        f++;
...
void usage() {
  ...
  printf(
      "Usage:  mppnccombine [-v] [-a] [-r] [-n #] [-e #] [-h #] [-64] [-m]\n");
  printf("                     output.nc [input ...]\n\n");
  printf("  -v    Print some progress information.\n");
  printf(
      "  -a    Append to an existing netCDF file (not heavily tested...).\n");
  printf(
      "  -r    Remove the \".####\" decomposed files after a successful "
      "run.\n");
  printf(
      "  -n #  Input filename extensions start with number #### instead of "
      "0000.\n");
  printf(
      "  -e #  Ending number #### of a specified range of input filename "
      "extensions.\n");
  printf(
      "        Files within the range do not have to be consecutively "
      "numbered.\n");
  printf(
      "  -h #  Add a specified number of bytes of padding at the end of the "
      "header.\n");
  printf("  -64   Create netCDF output files with the 64-bit offset format.\n");
  printf(
      "  -m    Initialize output variables with a \"missing_value\" from the "
      "variables\n");
  printf("        of the first input file instead of the default 0 value.\n\n");
  printf(
      "mppnccombine joins together an arbitrary number of netCDF input files, "
      "each\n");
  printf(
      "containing parts of a decomposed domain, into a unified netCDF output "
      "file.\n");
  printf(
      "An output file must be specified and it is assumed to be the first "
      "filename\n");
  printf(
      "argument.  If the output file already exists, then it will not be "
      "modified\n");
  printf(
      "unless the option is chosen to append to it.  If no input files are "
      "specified\n");
  printf(
      "then their names will be based on the name of the output file plus the "
      "default\n");
  printf(
      "numeric extension \".0000\", which will increment by 1.  There is an "
      "option for\n");
  printf(
      "starting the filename extensions with an arbitrary number instead of 0. "
      " There\n");
  printf(
      "is an option for specifying an end to the range of filename extension "
      "numbers;\n");
  printf(
      "files within the range do not have to be consecutively numbered.  If "
      "input\n");
  printf("files are specified then names will be used verbatim.\n\n");
  printf(
      "A value of 0 is returned if execution completed successfully; a value "
      "of 1\n");
  printf("otherwise.\n");
}
    ...
    for (v = 0; v < ncinfile->nvars; v++) {
      ...
      for (n = 0; n < ncinfile->natts[v]; n++) {
        ...
        if (!strcmp(attname, "domain_decomposition"))
          ...
          if (ncattcopy(ncinfile->ncfid, v, attname, ncoutfile->ncfid, v) ==
              (-1)) {
            fprintf(stderr,
...
int process_vars(struct fileinfo *ncinfile, struct fileinfo *ncoutfile,
                 unsigned char appendnc, int *nrecs, int r, int nfiles, int f,
                 void *varbuf[], unsigned char verbose, unsigned char missing) {
  ...
  else if (ncinfile->dimsize[ncinfile->recdim] != *nrecs) {
    fprintf(stderr,
    ...
    /* Save the buffer */
      ...
      if (_first_) {
        ...
        if ((varbuf[v] = calloc(varbufsize, 1)) == NULL) {
          fprintf(stderr,
                  "Error: cannot allocate %lld bytes for entire variable "
                  "\"%s\"'s values!\n",
                  varbufsize, ncinfile->varname[v]);
          return (1);
        }
        if (missing && ncoutfile->varmiss[v]) switch (ncinfile->datatype[v]) {
            case NC_BYTE:
            case NC_CHAR:
      ...
      imaxfull =
          ncinfile->dimfullsize[ncinfile->vardim[v][ncinfile->varndims[v] - 1]];
      if (ncinfile->varndims[v] > 1)
        jmaxfull =
            ncinfile
                ->dimfullsize[ncinfile->vardim[v][ncinfile->varndims[v] - 2]];
      else
        jmaxfull = 1;
      if (ncinfile->varndims[v] > 2)
        kmaxfull =
            ncinfile
                ->dimfullsize[ncinfile->vardim[v][ncinfile->varndims[v] - 3]];
      else
        kmaxfull = 1;
      if (ncinfile->varndims[v] > 3) {
        if (ncinfile->vardim[v][ncinfile->varndims[v] - 4] != ncinfile->recdim)
          lmaxfull =
              ncinfile
                  ->dimfullsize[ncinfile->vardim[v][ncinfile->varndims[v] - 4]];
        else
          lmaxfull = 1;
      } else
        lmaxfull = 1;
              ...
              for (j = 0; j < jmax; j++)
                for (i = 0; i < imax; i++) {
                  offset = (i + ioffset) + (j + joffset) * imaxfull +
                           (k + koffset) * imaxjmaxfull +
                           (l + loffset) * imaxjmaxkmaxfull;
                  *((short *)(varbuf[v]) + offset) = *((short *)values + (b++));
                }
              ...
              for (j = 0; j < jmax; j++)
                for (i = 0; i < imax; i++) {
                  offset = (i + ioffset) + (j + joffset) * imaxfull +
                           (k + koffset) * imaxjmaxfull +
                           (l + loffset) * imaxjmaxkmaxfull;
                  *((int *)(varbuf[v]) + offset) = *((int *)values + (b++));
                }
              ...
              for (j = 0; j < jmax; j++)
                for (i = 0; i < imax; i++) {
                  offset = (i + ioffset) + (j + joffset) * imaxfull +
                           (k + koffset) * imaxjmaxfull +
                           (l + loffset) * imaxjmaxkmaxfull;
                  *((float *)(varbuf[v]) + offset) = *((float *)values + (b++));
                }
...
int flush_decomp(struct fileinfo *ncoutfile, int nfiles, int r, void *varbuf[],
  ...
  for (v = 0; v < ncoutfile->nvars; v++) {
    ...
    if (verbose > 1) printf("      writing to disk\n");
