...
void PnetcdfOutput::WriteOutputFile(Mesh *pm, ParameterInput *pin, bool flag)
{
  ...
  for (int b = 0; b < pm->nblocal; ++b) {
    ...
    MPI_Offset start[4] = {0, ncells1*pmb->loc.lx1, ncells2*pmb->loc.lx2, ncells3*pmb->loc.lx3};
    ...
    if (nx1 > 1) {
      ...
      err = ncmpi_iput_vara_float(ifile, ivx1f, start+1, count1+1, *ib++, ir++);
      ERR
    }
    ...
    if (nx3 > 1) {
      ...
      err = ncmpi_iput_vara_float(ifile, ivx3f, start+3, count3+3, *ib++, ir++);
      ERR
    }
