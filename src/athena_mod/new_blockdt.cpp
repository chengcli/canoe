...
void Hydro::NewBlockTimeStep() {
  ...
  // Note, "dt_hyperbolic" currently refers to the dt limit imposed by evoluiton
  // of the ideal hydro or MHD fluid by the main integrator (even if not
  // strictly hyperbolic)
  Real min_dt_hyperbolic = real_max;
  // TODO(felker): consider renaming dt_hyperbolic after general execution model
  // is implemented and flexibility from #247 (zero fluid configurations) is
  // addressed. dt_hydro, dt_main (inaccurate since "dt" is actually main),
  // dt_MHD?
  Real min_dt_parabolic = real_max;
  Real min_dt_user = real_max;

  // TODO(felker): skip this next loop if pm->fluid_setup ==
  // FluidFormulation::disabled
...
#pragma ivdep
        for (int i = is; i <= ie; ++i) {
          ...
          if (fluid_status == FluidFormulation::evolve) {
            if (MAGNETIC_FIELDS_ENABLED) {
              AthenaArray<Real> &bcc = pmb->pfield->bcc,
                                &b_x1f = pmb->pfield->b.x1f,
                                &b_x2f = pmb->pfield->b.x2f,
                                &b_x3f = pmb->pfield->b.x3f;
              Real bx = bcc(IB1, k, j, i) +
                        std::abs(b_x1f(k, j, i) - bcc(IB1, k, j, i));
              ...
              bx = bcc(IB2, k, j, i) +
                   std::abs(b_x2f(k, j, i) - bcc(IB2, k, j, i));
              ...
              bx = bcc(IB3, k, j, i) +
                   std::abs(b_x3f(k, j, i) - bcc(IB3, k, j, i));
              cf = pmb->peos->FastMagnetosonicSpeed(wi, bx);
              dt3(i) /= (std::abs(wi[IVZ]) + cf);
            } else {
              Real cs = pmb->peos->SoundSpeed(wi);
              int implicit_flag = pmb->pimpl->phevi->implicit_flag;
          ...
          } else {  // FluidFormulation::background or disabled. Assume scalar
                    // advection:
  ...
  if (MAGNETIC_FIELDS_ENABLED && pmb->pfield->fdif.field_diffusion_defined) {
  ...
  // parabolic:
  // STS handles parabolic terms -> then take the smaller of hyperbolic or user
  // timestep
  if (!STS_ENABLED) {
