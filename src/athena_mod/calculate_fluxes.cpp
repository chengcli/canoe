...
void Hydro::CalculateFluxes(AthenaArray<Real> &w, FaceField &b,
  ...
  // b,bcc are passed as fn parameters becausse clients may want to pass
  // different bcc1, b1, b2, etc., but the remaining members of the Field class
  // are accessed directly via pointers because they are unique. NOTE: b, bcc
  // are nullptrs if no MHD.
  ...
  for (int k = kl; k <= ku; ++k) {
    for (int j = jl; j <= ju; ++j) {
      ...
      if (order == 1) {
        ...
        pmb->pimpl->precon->Weno5X1(k, j, is - 1, ie + 1, w, bcc, wl_, wr_);
      } else {
        pmb->precon->PiecewiseParabolicX1(k, j, is - 1, ie + 1, w, bcc, wl_,
                                          wr_);
      }
      ...
      RiemannSolver(k, j, is, ie + 1, IVX, b1, wl_, wr_, x1flux, e3x1, e2x1,
                    w_x1f, dxw_);
#endif
      if (order == 4) {
  ...
  if (order == 4) {
    // TODO(felker): assuming uniform mesh with dx1f=dx2f=dx3f, so this should
    // factor out
    // TODO(felker): also, this may need to be dx1v, since Laplacian is
    // cell-centered
    Real h = pmb->pcoord->dx1f(is);  // pco->dx1f(i); inside loop
    Real C = (h * h) / 24.0;

    // construct Laplacian from x1flux
    pmb->pcoord->LaplacianX1All(x1flux, laplacian_all_fc, 0, NHYDRO - 1, kl, ku,
                                jl, ju, is, ie + 1);
...
#pragma omp simd
        ...
        // TODO(felker): check that e3x1,e2x1 arguments added in late 2017 work
        // here
        pmb->pcoord->CenterWidth1(k, j, is, ie + 1, dxw_);
#if !MAGNETIC_FIELDS_ENABLED  // Hydro:
        ...
        RiemannSolver(k, j, is, ie + 1, IVX, b1, wl_, wr_, flux_fc, e3x1, e2x1,
                      w_x1f, dxw_);
#endif
        // Apply Laplacian of second-order accurate face-averaged flux on x1
        // faces
        for (int n = 0; n < NHYDRO; ++n) {
#pragma omp simd
          for (int i = is; i <= ie + 1; i++) {
            x1flux(n, k, j, i) =
                flux_fc(n, k, j, i) + C * laplacian_all_fc(n, k, j, i);
            // TODO(felker): replace this loop-based deep copy with memcpy, or
            // alternative
            if (n == IDN && NSCALARS > 0) {
  ...
  if (pmb->pmy_mesh->f2) {
    ...
    for (int k = kl; k <= ku; ++k) {
      ...
      if (order == 1) {
        ...
        pmb->pimpl->precon->Weno5X2(k, js - 1, il, iu, w, bcc, wl_, wr_);
        for (int i = il; i <= iu; ++i) {
      ...
      for (int j = js; j <= je + 1; ++j) {
        ...
        if (order == 1) {
          ...
          pmb->pimpl->precon->Weno5X2(k, j, il, iu, w, bcc, wlb_, wr_);
          for (int i = il; i <= iu; ++i) {
            pmb->peos->ApplyPrimitiveFloors(wl_, k, j, i);
            pmb->peos->ApplyPrimitiveFloors(wr_, k, j, i);
          }
       ...
       // flx(IBZ) = (v2*b1 - v1*b2) =  EMFZ
        RiemannSolver(k, j, il, iu, IVY, b2, wl_, wr_, x2flux, e1x2, e3x2,
                      w_x2f, dxw_);
#endif
    ...
    if (order == 4) {
      // TODO(felker): assuming uniform mesh with dx1f=dx2f=dx3f, so factor this
      // out
      // TODO(felker): also, this may need to be dx2v, since Laplacian is
      // cell-centered
      Real h = pmb->pcoord->dx2f(js);  // pco->dx2f(j); inside loop
      Real C = (h * h) / 24.0;

      // construct Laplacian from x2flux
      pmb->pcoord->LaplacianX2All(x2flux, laplacian_all_fc, 0, NHYDRO - 1, kl,
                                  ku, js, je + 1, il, iu);
...
#pragma omp simd
          ...
          // TODO(felker): check that e1x2,e3x2 arguments added in late 2017
          // work here
          pmb->pcoord->CenterWidth2(k, j, il, iu, dxw_);
#if !MAGNETIC_FIELDS_ENABLED  // Hydro:
...
#pragma omp simd
            for (int i = il; i <= iu; i++) {
              x2flux(n, k, j, i) =
                  flux_fc(n, k, j, i) + C * laplacian_all_fc(n, k, j, i);
              if (n == IDN && NSCALARS > 0) {
                pmb->pscalars->mass_flux_fc[X2DIR](k, j, i) =
                    flux_fc(n, k, j, i);
              }
            }
          }
  ...
  if (pmb->pmy_mesh->f3) {
    ...
    for (int j = jl; j <= ju; ++j) {  // this loop ordering is intentional
      ...
      if (order == 1) {
        ...
        pmb->pimpl->precon->Weno5X3(ks - 1, j, il, iu, w, bcc, wl_, wr_);
        for (int i = il; i <= iu; ++i) {
      ...
      for (int k = ks; k <= ke + 1; ++k) {
        ...
        if (order == 1) {
          ...
          pmb->pimpl->precon->Weno5X3(k, j, il, iu, w, bcc, wlb_, wr_);
          for (int i = il; i <= iu; ++i) {
            pmb->peos->ApplyPrimitiveFloors(wl_, k, j, i);
            pmb->peos->ApplyPrimitiveFloors(wr_, k, j, i);
          }
       ...
       // flx(IBZ) = (v3*b2 - v2*b3) =  EMFX
        RiemannSolver(k, j, il, iu, IVZ, b3, wl_, wr_, x3flux, e2x3, e1x3,
                      w_x3f, dxw_);
#endif
        if (order == 4) {
          for (int n = 0; n < NWAVE; n++) {
    ...
    if (order == 4) {
      // TODO(felker): assuming uniform mesh with dx1f=dx2f=dx3f, so factor this
      // out
      // TODO(felker): also, this may need to be dx3v, since Laplacian is
      // cell-centered
      Real h = pmb->pcoord->dx3f(ks);  // pco->dx3f(j); inside loop
      Real C = (h * h) / 24.0;

      // construct Laplacian from x3flux
      pmb->pcoord->LaplacianX3All(x3flux, laplacian_all_fc, 0, NHYDRO - 1, ks,
                                  ke + 1, jl, ju, il, iu);
...
#pragma omp simd
          ...
          // TODO(felker): check that e2x3,e1x3 arguments added in late 2017
          // work here
          pmb->pcoord->CenterWidth3(k, j, il, iu, dxw_);
#if !MAGNETIC_FIELDS_ENABLED  // Hydro:
          ...
          RiemannSolver(k, j, il, iu, IVZ, b3, wl_, wr_, flux_fc, e2x3, e1x3,
                        w_x3f, dxw_);
#endif
          // Apply Laplacian of second-order accurate face-averaged flux on x3
          // faces
          for (int n = 0; n < NHYDRO; ++n) {
#pragma omp simd
            for (int i = il; i <= iu; i++) {
              x3flux(n, k, j, i) =
                  flux_fc(n, k, j, i) + C * laplacian_all_fc(n, k, j, i);
              if (n == IDN && NSCALARS > 0) {
                pmb->pscalars->mass_flux_fc[X3DIR](k, j, i) =
                    flux_fc(n, k, j, i);
              }
            }
          }
  ...
  if (!STS_ENABLED) AddDiffusionFluxes();
...
void Hydro::CalculateFluxes_STS() { AddDiffusionFluxes(); }
