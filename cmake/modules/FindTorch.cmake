...
macro(__torch_determine_version)
  execute_process(
    ...
    OUTPUT_VARIABLE TORCH_PEP440_VERSION
    OUTPUT_STRIP_TRAILING_WHITESPACE
    RESULT_VARIABLE result)

  if(result GREATER 0)
    message(
      FATAL_ERROR "fairseq2n cannot determine PEP 440 version of PyTorch!")
  endif()
...
if(FAIRSEQ2_USE_LIBTORCH)
  ...
  message(FATAL_ERROR "`libtorch` is not supported yet.")
endif()
...
find_library(
  ...
  NO_DEFAULT_PATH)
find_library(
  TORCH_CPU_LIBRARY torch_cpu
  HINTS ${torch_lib_dir}
  NO_DEFAULT_PATH)
find_library(
  TORCH_CUDA_LIBRARY torch_cuda
  HINTS ${torch_lib_dir}
  NO_DEFAULT_PATH)
...
find_path(
  ...
  NO_DEFAULT_PATH)
find_path(
  TORCH_API_INCLUDE_DIR torch
  HINTS ${torch_api_include_dir}
  NO_DEFAULT_PATH)

set(torch_required_vars TORCH_LIBRARY TORCH_CPU_LIBRARY C10_LIBRARY
...
find_package_handle_standard_args(
  Torch
  REQUIRED_VARS ${torch_required_vars}
...
if(TORCH_CUDA_LIBRARY)
  ...
  set(TORCH_VARIANT
      "CUDA ${TORCH_CUDA_VERSION_MAJOR}.${TORCH_CUDA_VERSION_MINOR}")
else()
  ...
  if(CMAKE_SYSTEM_NAME STREQUAL "Linux")
    ...
    if(result EQUAL 0)
      target_compile_definitions(
        torch_cxx11_abi
        INTERFACE _GLIBCXX_USE_CXX11_ABI=$<BOOL:${TORCH_CXX11_ABI}>)
    endif()
...
if(NOT TARGET torch_python)
  ...
  set_property(TARGET torch_python PROPERTY IMPORTED_LOCATION
                                            ${TORCH_PYTHON_LIBRARY})
